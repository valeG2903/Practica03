...
public class RevistaMySqlRepositorio implements RevistaRepositorio{
	...
	public List<Revista> listaRevista() {
		List<Revista> lista= jdbcTemplate.query("select r.*,c.nombre from revista r inner join categoria c on r.idCategoria = d.idcategoria", new Object[] {} ,mapperRevista);
		return lista;
	}
	...
	public Revista insertaActualizaRevista(Revista obj) {
		...
		if (obj.getIdRevista() == 0) {
			...
			new Object[] {obj.getNombre(), obj.getPrecio(), obj.getSerie(),obj.getFechaLanzamiento(),obj.getFechaEdicion(),obj.getCategoria()});	
